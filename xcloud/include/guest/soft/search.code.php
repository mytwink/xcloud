...
if($len > 2) {
    ...
    $res = DB::query("SELECT `id`, `title`, `des`, `img` FROM `soft` WHERE `tag` LIKE ? OR `tag` LIKE ? OR `title` LIKE ? OR `title` LIKE ?",
        [$key1,$key2,$key1,$key2]);

    if($res->rowCount() == 0)
        $res = DB::query("SELECT `id`, `title`, `des`, `img` FROM `soft` WHERE `des` LIKE ? OR `des` LIKE ?",
            [$key1, $key2]);
