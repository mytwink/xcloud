...
if($len > 2) {
    ...
    $res = DB::query("SELECT `id`, `title`, `des`, `img`, `type` FROM `media` WHERE `tag` LIKE ? OR `tag` LIKE ? OR `title` LIKE ? OR `title` LIKE ?",
        [$key1,$key2,$key1,$key2]);

    if($res->rowCount() == 0)
        $res = DB::query("SELECT `id`, `title`, `des`, `img`, `type` FROM `media` WHERE `des` LIKE ? OR `des` LIKE ?",
            [$key1, $key2]);
