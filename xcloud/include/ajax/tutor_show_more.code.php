...
if($page_type == 0)
$res = DB::query('SELECT `id`, `title`, `des`, `img` FROM `tutor` ORDER BY `added` DESC LIMIT ?,4',[$id])->fetchAll(PDO::FETCH_ASSOC);

//Для страницы по жанрам
else if(is_numeric($page_type))
$res = DB::query('SELECT `id`, `title`, `des`, `img` FROM `tutor` WHERE `cat` LIKE ? ORDER BY `added` DESC LIMIT ?,4',
